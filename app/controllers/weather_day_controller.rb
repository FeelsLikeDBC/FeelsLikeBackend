  ...
  def weather
    ...
    @days = Array.new(366) do |i|
      ...
      end
      {
        ...
        cloud_cover: days_array.map { |day| day.cloud_cover }.reduce(:+)/days_array.length.round(2),
        avg_apparent_day_temp: days_array.map { |day| day.avg_apparent_day_temp }.reduce(:+)/days_array.length.round(2),
        avg_apparent_night_temp: days_array.map { |day| day.avg_apparent_night_temp }.reduce(:+)/days_array.length.round(2),
        high_apparent_temp: days_array.map { |day| day.high_apparent_temp }.reduce(:+)/days_array.length.round(2),
        low_apparent_temp: days_array.map { |day| day.low_apparent_temp }.reduce(:+)/days_array.length.round(2),
